...
//
// Subtracts gravity from accelerometer readings and bias from gyroscope
// readings so you only see dynamic movement. On startup the sensor must
// be stationary for calibration — it averages several samples to learn
// the gravity vector and gyro bias, then subtracts them from every
// subsequent reading.
//
...
// Calibration baselines (raw int16 values)
...
void calibrate() {
  ...
  uint16_t accel_collected = 0, gyro_collected = 0;

  while (accel_collected < CAL_SAMPLES || gyro_collected < CAL_SAMPLES) {
    ...
    while (sox.readFIFOWord(tag, x, y, z)) {
      if (tag == LSM6DS_FIFO_TAG_ACCEL_NC && accel_collected < CAL_SAMPLES) {
        accel_sum_x += x;
        accel_sum_y += y;
        accel_sum_z += z;
        accel_collected++;
      } else if (tag == LSM6DS_FIFO_TAG_GYRO_NC && gyro_collected < CAL_SAMPLES) {
        gyro_sum_x += x;
        gyro_sum_y += y;
        gyro_sum_z += z;
        gyro_collected++;
  ...
  gravity_x = (float)accel_sum_x / CAL_SAMPLES;
  gravity_y = (float)accel_sum_y / CAL_SAMPLES;
  gravity_z = (float)accel_sum_z / CAL_SAMPLES;

  gyro_bias_x = (float)gyro_sum_x / CAL_SAMPLES;
  gyro_bias_y = (float)gyro_sum_y / CAL_SAMPLES;
  gyro_bias_z = (float)gyro_sum_z / CAL_SAMPLES;
...
void loop() {
  ...
  while (sox.readFIFOWord(tag, x, y, z)) {
    ...
    case LSM6DS_FIFO_TAG_GYRO_NC:
      gyro_x = (int16_t)(x - gyro_bias_x);
      gyro_y = (int16_t)(y - gyro_bias_y);
      gyro_z = (int16_t)(z - gyro_bias_z);
      break;
